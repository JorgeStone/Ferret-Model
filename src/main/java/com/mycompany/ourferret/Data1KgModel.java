    ...
    public void variantProcess(SettingsModel settings, LinkedList<ElementSaisiModel> snpQueries) {
        ...
        try {
            ...
            for (int i = 0; i < snpQueries.size(); i++) {
                ...
                boolean chromosomeFound = false, startFound = false, endFound = false, locatedOnInvalidChr = false;
                while (!(startFound && endFound && chromosomeFound) && (currentString = br.readLine()) != null) {
...
// getPeopleString_Phase3_GRCh38
    public static String getPeopleString_Phase3_GRCh38(String NumOfChr) throws IOException {
        String result = null;
        try {
            String webAddress = "http://ftp.1000genomes.ebi.ac.uk/vol1/ftp/release/20130502/supporting/GRCh38_positions/ALL.chr" + NumOfChr + ".phase3_shapeit2_mvncall_integrated_v3plus_nounphased.rsID.genotypes.GRCh38_dbSNP_no_SVs.vcf.gz";
...
// getPeopleString_Phase1 fonctionne bien
    public static String getPeopleString_Phase1(String NumOfChr) throws IOException {
        String result = null;
        try {
            String webAddress = "http://ftp.1000genomes.ebi.ac.uk/vol1/ftp/release/20110521/ALL.chr" + NumOfChr + ".phase1_release_v3.20101123.snps_indels_svs.genotypes.vcf.gz";
            TabixReader tabReader = new TabixReader(webAddress);
            result = tabReader.readLine();
            while (!result.contains("CHROM")) {
                result = tabReader.readLine();
    ...
    public void process1KG(SettingsModel settings, LinkedList<ElementSaisiModel> enteredElements) throws IOException {

        if (enteredElements != null && enteredElements.get(0).getClass().getSimpleName().equals("VariantModel")) {
            variantProcess(settings, enteredElements);
        } else if (enteredElements != null && enteredElements.get(0).getClass().getSimpleName().startsWith("GeneModel")) {
        ...
        for (int i = 0; i < sortedLocus.size(); i++) {
            querySize[i + 1] = querySize[i] + (sortedLocus.get(i).getEnd() - sortedLocus.get(i).getStart());
        }
        try {
            String s;
            BufferedReader popBuffRead = null;
            if (ftpAdress.contains("qlq chose qui précise qu'on veut travailler sur la phase 1")) {
                popBuffRead = new BufferedReader(new InputStreamReader(Data1KgModel.class.getClassLoader().getResourceAsStream("samplesPhase1.txt")));
            } else {
                popBuffRead = new BufferedReader(new InputStreamReader(Data1KgModel.class.getClassLoader().getResourceAsStream("samplesPhase3.txt")));
            }
            // Création des population intéressé par l'utilisateur avec leurs indices dans la liste PeopleOfInterest
            String[] IDs;
            ...
            if (!enteredElements.get(0).contain("ALL")) {
                while (s != null) {
                    IDs = s.split("\t");
                    if (enteredElements.get(0).contain(IDs[1]) || enteredElements.get(0).contain(IDs[2])) {
        ...
        if (settings.getOutput().equals("vcf")) {
            ...
            try {
                ...
                for (int j = 0; j < queryNumber; j++) {
                    ...
                    TabixReader.Iterator iter = treader.query(sortedLocus.get(j).getChromosome() + ":" + sortedLocus.get(j).getStart() + "-" + sortedLocus.get(j).getEnd());
                    ...
                    while ((s = iter.next()) != null) {
                        ...
                        if (isInt(stringSplit[1])) {
                            tempInt = Integer.parseInt(stringSplit[1]) - sortedLocus.get(j).getStart();
                            if (tempInt > 0) {
            ...
            try {
                ...
                for (int j = 0; j < queryNumber; j++) {
                    ...
                    // Get the iterator
                    startTime = System.nanoTime();
                    TabixReader.Iterator iter = tr.query(sortedLocus.get(j).getChromosome() + ":" + sortedLocus.get(j).getStart() + "-" + sortedLocus.get(j).getEnd());
                    long endTime = System.nanoTime();
                    System.out.println("Tabix iterator time: " + (endTime - startTime));
                    while ((s = iter.next()) != null) {
                        countVariant++;
                        ...
                        if (stringSplit[6].equals("PASS")) {
                            ...
                            if (stringSplit[2].equals(".")) {
                                stringSplit[2] = "chr" + sortedLocus.get(j).getChromosome() + "_" + stringSplit[1];
        ...
        try {
            ...
            BufferedReader familyInfoRead = new BufferedReader(new InputStreamReader(Data1KgModel.class.getClassLoader().getResourceAsStream("family_info.txt")));
